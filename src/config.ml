...
let t_of_sexp dsa t =
  ...
  | Sexp.List l ->
    (match
      List.fold_left (fun (ver, jid, prio, host, port, pass, auth, cert, notification) -> function
          | Sexp.List [ Sexp.Atom "version" ; v ] ->
            assert (ver = None) ;
            let version = int_of_sexp v in
            (Some version, jid, prio, host, port, pass, auth, cert, notification)
          | Sexp.List [ Sexp.Atom "jid" ; Sexp.Atom v ] ->
            ...
            in
            (ver, Some jid, prio, host, port, pass, auth, cert, notification)
          | Sexp.List [ Sexp.Atom "priority" ; p ] ->
            assert (prio = None) ;
            let prio = option_of_sexp int_of_sexp p
            in
            (ver, jid, Some prio, host, port, pass, auth, cert, notification)
          | Sexp.List [ Sexp.Atom "hostname" ; h ] ->
            assert (host = None) ;
            let host = option_of_sexp string_of_sexp h in
            (ver, jid, prio, Some host, port, pass, auth, cert, notification)
          | Sexp.List [ Sexp.Atom "port" ; p ] ->
            ...
            in
            (ver, jid, prio, host, Some port, pass, auth, cert, notification)
          | Sexp.List [ Sexp.Atom "password" ; password ] ->
            ...
            in
            (ver, jid, prio, host, port, Some pass, auth, cert, notification)
          | Sexp.List [ Sexp.Atom "trust_anchor" ; trust_anchor ] ->
            assert (auth = None) ;
            (match ver with
             | Some 0 ->
               let auth = Some (`Trust_anchor (string_of_sexp trust_anchor)) in
               (ver, jid, prio, host, port, pass, auth, cert, notification)
             | Some 1 -> ( match option_of_sexp string_of_sexp trust_anchor with
                 | None -> (ver, jid, prio, host, port, pass, auth, cert, notification)
                 | Some x -> (ver, jid, prio, host, port, pass, Some (`Trust_anchor x), cert, notification) )
            ...
            ( match ver with
              | Some 1 ->
                ( match option_of_sexp string_of_sexp tls_fp with
                  | None -> (ver, jid, prio, host, port, pass, auth, cert, notification)
                  | Some x -> (ver, jid, prio, host, port, pass, Some (`Fingerprint x), cert, notification) )
              | _ -> raise (Invalid_argument "unexpected element: tls_fingerprint") )
          | Sexp.List [ Sexp.Atom "authenticator" ; authenticator ] ->
            assert (auth = None) ;
            (ver, jid, prio, host, port, pass, Some (auth_of_sexp authenticator), cert, notification)
     ...
     with
     | Some version, Some jid, Some priority, Some hostname, Some port, password, Some authenticator, certificate_hostname, notification_callback ->
       { version ; jid ; priority ; hostname ; port ; password ; authenticator ; otr_config ; dsa ; certificate_hostname ; notification_callback }
     | Some version, Some jid, None, Some hostname, Some port, password, Some authenticator, certificate_hostname, notification_callback ->
       { version ; jid ; priority = None ; hostname ; port ; password ; authenticator ; otr_config ; dsa ; certificate_hostname ; notification_callback }
     | Some version, Some jid, Some priority, None, Some port, password, Some authenticator, certificate_hostname, notification_callback ->
       { version ; jid ; priority ; hostname = None ; port ; password ; authenticator ; otr_config ; dsa ; certificate_hostname ; notification_callback }
     | Some version, Some jid, None, None, Some port, password, Some authenticator, certificate_hostname, notification_callback ->
       { version ; jid ; priority = None ; hostname = None ; port ; password ; authenticator ; otr_config ; dsa ; certificate_hostname ; notification_callback }
     | _ -> raise (Invalid_argument "broken config") )
  | _ -> raise (Invalid_argument "broken config")
