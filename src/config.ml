...
let t_of_sexp dsa t =
  ...
  | Sexp.List l ->
    (match
      List.fold_left (fun (ver, jid, prio, host, port, pass, auth, cert) -> function
          | Sexp.List [ Sexp.Atom "version" ; v ] ->
            assert (ver = None) ;
            let version = int_of_sexp v in
            (Some version, jid, prio, host, port, pass, auth, cert)
          ...
          | Sexp.List [ Sexp.Atom "port" ; p ] ->
            ...
            in
            (ver, jid, prio, host, Some port, pass, auth, cert)
          | Sexp.List [ Sexp.Atom "password" ; password ] ->
            ...
            in
            (ver, jid, prio, host, port, Some pass, auth, cert)
          | Sexp.List [ Sexp.Atom "trust_anchor" ; trust_anchor ] ->
            assert (auth = None) ;
            (match ver with
             | Some 0 ->
               let auth = Some (`Trust_anchor (string_of_sexp trust_anchor)) in
               (ver, jid, prio, host, port, pass, auth, cert)
             | Some 1 -> ( match option_of_sexp string_of_sexp trust_anchor with
                 | None -> (ver, jid, prio, host, port, pass, auth, cert)
                 | Some x -> (ver, jid, prio, host, port, pass, Some (`Trust_anchor x), cert) )
            ...
            ( match ver with
              | Some 1 ->
                ( match option_of_sexp string_of_sexp tls_fp with
                  | None -> (ver, jid, prio, host, port, pass, auth, cert)
                  | Some x -> (ver, jid, prio, host, port, pass, Some (`Fingerprint x), cert) )
              | _ -> raise (Invalid_argument "unexpected element: tls_fingerprint") )
          | Sexp.List [ Sexp.Atom "authenticator" ; authenticator ] ->
            assert (auth = None) ;
            (ver, jid, prio, host, port, pass, Some (auth_of_sexp authenticator), cert)
     ...
     with
     | Some version, Some jid, Some priority, Some hostname, Some port, password, Some authenticator, certificate_hostname ->
       { version ; jid ; priority ; hostname ; port ; password ; authenticator ; otr_config ; dsa ; certificate_hostname }
     | Some version, Some jid, None, Some hostname, Some port, password, Some authenticator, certificate_hostname ->
       { version ; jid ; priority = None ; hostname ; port ; password ; authenticator ; otr_config ; dsa ; certificate_hostname }
     | Some version, Some jid, Some priority, None, Some port, password, Some authenticator, certificate_hostname ->
       { version ; jid ; priority ; hostname = None ; port ; password ; authenticator ; otr_config ; dsa ; certificate_hostname }
     | Some version, Some jid, None, None, Some port, password, Some authenticator, certificate_hostname ->
       { version ; jid ; priority = None ; hostname = None ; port ; password ; authenticator ; otr_config ; dsa ; certificate_hostname }
     | _ -> raise (Invalid_argument "broken config") )
  | _ -> raise (Invalid_argument "broken config")
