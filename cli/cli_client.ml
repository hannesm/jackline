  ...
  let hline =
    let buddy, pres = match state.active_chat with
      | u, Some s ->
        ...
        let otr, raw = User.fingerprint s.User.otr in
        let verified = if User.verified_fp u raw then "verified" else "not verified" in
        (User.userid u s, " - " ^ p ^ status ^ " " ^ otr ^ " " ^ verified)
      | u, None -> (u.User.jid, "")
    in
